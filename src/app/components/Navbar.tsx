    ...
    <>
      <nav className="w-full px-4 md:px-6 py-3 md:py-5 bg-black text-white flex items-center fixed top-0 z-50 border-b border-gray-800 shadow-lg h-[60px] md:h-[70px]">
        ...
        <div className="flex items-center">
          ...
          {/* Mobile Cart and Trial Room - positioned next to logo */}
          <div className="md:hidden ml-4 flex items-center gap-3">
            {/* Trial Room Button - Mobile */}
            {cartItems.length > 0 && (
              <button
                onClick={() => setShowTrialRoom(true)}
                className="px-2 py-1 bg-white text-black rounded-full text-xs hover:bg-gray-100 transition-colors duration-200 whitespace-nowrap"
              >
                Trial Room
              </button>
            )}
